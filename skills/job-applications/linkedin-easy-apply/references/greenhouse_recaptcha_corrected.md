# Greenhouse guest-board flow — CORRECTED (XXXXXXX, session-verified)
...
## CORRECT FIELD INTERACTION (the XXXXXXX / search-dropdown fix — user's correction)
...
XXXXXXX country-code flag.

WRONG (what failed):
- `setR('#country','XXXXXXX')` → sets raw value that never commits (stays "").
- typing "XXXXXXX" into the dropdown → matches **"British XXXXXXXn Ocean Territory +246"**
  (starts with "Indi"!) → wrong country.

CORRECT:
1. Country: click `#country` input → `page.keyboard.type('XXXXXXX',{delay:45})` →
   click the `[role=option]`/`.select__option` whose text **includes "XXXXXXX"** (not just "XXXXXXX").
...
2. Phone: click `.iti__selected-flag` → focus `#iti-0__search-input` → type "XXXXXXX" →
   pick the `.iti__country` option with `XXXXXXX` → then clear `#phone` and type full
   number `XXXXXXX_NUMBER` (no separate XXXXXXX prefix needed; the flag carries it).
...
## LINKEDIN_PORT tab hygiene — DO NOT mass-close
- Mass-closing 20+ tabs via `/json/close` loop CRASHED the LINKEDIN_PORT Chrome (connection refused)
  this session. The LINKEDIN_PORT session is fragile under tab operations.
- Drive ONE tab at a time (skill's single-tab rule). If you must reduce tabs, close a FEW
  with delays, never a tight loop.
- Relaunch LINKEDIN_PORT (loses LinkedIn login — operator re-auths) with:
...
`gh_batch.cjs` on the ATS_PORT anti-detect Chrome (`cdp_helper_9223.cjs`):
...
- ATS_PORT launch: `chrome.exe --remote-debugging-port=ATS_PORT --user-data-dir=XXXXXXX/greenhouse-chrome
  --disable-blink-features=AutomationControlled` (keeps LINKEDIN_PORT LinkedIn untouched).
