# External ATS Orchestration Playbook (VERIFIED 2026-08-19 — corrected)

LinkedIn jobs that are NOT Easy Apply redirect off-platform ("Responses managed off
LinkedIn"). These are the external ATS types seen in the wild for operator's stack:

| ATS / host | Example | Autonomy level | Notes |
|---|---|---|---|
| **Greenhouse** | `job-boards.greenhouse.io/<company>` | GUEST board fill + **HUMAN submit** | reCAPTCHA silently drops automated submit — Alfred fills 100%, operator clicks Submit. See GREENHOUSE BOARD FLOW below. |
| **Google Form** | `docs.google.com/forms/...` | FULL | Alfred has Google auth; deterministic field fill. Easiest external. **BUT scam-check first** (see Safety). |
...
| **Workday** | `*.myworkdayjobs.com/...` | MANUAL-HEAVY | Strong bot defense, often needs an account + manual steps. STOP + tell operator. |

## GREENHOUSE BOARD FLOW (verified 2026-08-19 — the login path is DEAD)

### Reality check: the logged-in candidate portal does NOT work
- `app.greenhouse.io` and the saved cookies (`greenhouse_session_cookies.json`, 19
  ...
  application" / saved-profile — it needs live login (operator's interactive Google SSO).
...
1. **Discover**: `web_search("boards.greenhouse.io <role> <stack> remote")`. Live slugs
   found 2026-08-19: `lumimeds` (43 jobs), `techholding` (19 jobs, incl. Senior
   FullStack Engineer Ahmedabad India), `remotecom`, `earnin`, `leaflink`, `reltio`.
...
3. **SUBMIT TRAP (cost 4 wasted runs 2026-08-19):** TWO buttons exist.
...
4. **Field-fill techniques (verified to stick):**
   ...
   - **react-select dropdowns** (`#country`, `#degree--0`, Yes/No questions): NOT native
     ...
     `page.keyboard.press('Enter')`. For Country type "India", Degree "Bachelor's",
     Yes/No "Yes". A plain `setReact('#country','India')` does NOT commit (dropdown stays
   ...
   - **Resume**: `page.$('#resume')` then `handle.uploadFile(RESUME_PATH)`. The CDP
     ...
     `operator_Biswas_Resume_ATS.pdf` shows with a remove-X before submitting.
5. **reCAPTCHA IS A REAL BLOCKER** (2026-08-19, VERIFIED): an automated CDP "Submit
   ...
   **WORKING WORKAROUND**: Alfred fills 100% (vision-verified, resume attached) and then
...
## SAFETY GATE (release-blocking, operator 2026-08-19 — "beware scam jobs / don't hand over bank details")
...
1. **Never submit banking/payment data.** If a field matches
   ...
   AND it isn't obviously salary expectation → STOP, flag it. Numeric CTC (8600000/5000000) is ALLOWED.
...
3. Real case blocked 2026-08-19: a Google Form "Candidate Internship Form" by "Skillfied
...
1. Navigate to the real ATS URL in the 9222 tab (single-tab rule applies).
...
3. Extract every question → Alfred reasons each answer from the verified profile → fill via
...
6. Handle gates:
   ...
   - **Visible captcha / "verify you are human"** → STOP, tell operator.
...
secret = "OPERATOR_HOME/AppData/Local/hermes/google_client_secret.json"
...
json.dump({"code_verifier": flow.code_verifier}, open("OPERATOR_HOME/job-apply/_gws_verifier.json","w"))
# give auth_url to operator; he approves and pastes the on-screen code
ver = json.load(open("OPERATOR_HOME/job-apply/_gws_verifier.json"))["code_verifier"]
flow.code_verifier = ver
flow.fetch_token(code=CODE_PASTED_BY_USER)
open("OPERATOR_HOME/AppData/Local/hermes/google_token.json","w").write(flow.credentials.to_json())
```
...
- Live token: `OPERATOR_HOME/AppData/Local/hermes/google_token.json` (authoritative).
...
- The 9222 single-tab rule and vision-confirm rules from SKILL.md apply identically.
