...
metadata:
  hermes:
    tags: [Job Search, Resume, Cover Letter, Referral, Outreach, ATS, Okta, Identity]
    category: productivity
---

# Job Applications (OPERATOR_NAME / operator)
...
## operator's durable profile (use for targeting)
- Senior/Staff Full Stack + Applied AI SWE, 14+ yrs. Bengaluru/Kolkata-based, authorized to work in India, open to hybrid/remote.
- DEEP specialty = **Identity / IAM / KYC / OAuth2 / OIDC** (Maersk Spot Award for an OIDC component that cut SSO failures 28%; Appsian + EXILANT enterprise security for Apple/SingPost). This is his #1 differentiator.
...
- Domains: aviation (Batik Air flight-ops, Sabre GDS), maritime (Maersk), logistics, cybersecurity, enterprise SaaS, applied AI (Wraith, local LLMs/Ollama).
- **Last CTC: INR 86 LPA (`8600000`/yr). THIS IS THE FLOOR.** Never quote a current
  CTC below 86L, and never an expected CTC at or below it, on any form or in any
  negotiation. Expected ask ~1.15-1.4x, i.e. roughly 100L-120L (`11000000` is the
  default expected figure). **The older "~48L take-home floor" note was WRONG and is
  revoked** — it caused two real applications to go out under-quoted on 2026-08-07
...
## Workflow
1. **Deep research (parallel):** his resume + GitHub (`api.github.com/users/YOUR_GITHUB_USERNAME`), portfolio (operatorbiswas.twinesite.com), Gmail job stream, and the live market. Gmail: use `gmail-automation` skill, BYPASS the gws shim (strip `~/bin` from PATH), search `job OR hiring OR recruiter OR referral newer_than:150d`.
...
3. **Tailor resume + cover letter** from his real data. Re-lead with the matching specialty; frame 14 yrs at Staff level.
...
- **LinkedIn interaction IS permitted via Composio (updated Aug 2026).** operator authorized Alfred to use Composio Connect MCP or Composio CLI for LinkedIn: connection requests, messaging, and referral asks (e.g. the Okta / Belinda Saffioti referral flow). Auth is handled through Composio (NOT raw LinkedIn creds), so it is allowed where raw automation was not. For PUBLIC posts/comments, still draft-then-confirm with operator before sending. See `references/linkedin-composio.md` for setup + the venv-isolation rule. The old "never touch LinkedIn" rule is REVOKED.
- **GitHub hygiene is mandatory before applying to security/employer.** Okta is a security firm AND a US federal contractor that background-checks. Set attack-tool repos (`Nuke`, `LOIC`, `MHDDoS`, `byob`, `trape`, etc.) and Anonymous-op bios to private. Keep `Wraith`, `Wraith Engage`, `fake-job-email-response-guide` public.
- **Don't fabricate backend language.** Okta JD prefers Go; operator is Node.js/TypeScript. Frame Node as his backend, show ramp ability, never claim Go/Java production experience.
- **Don't use Wraith (or any AI copilot) in the actual interview** — integrity line, and they build the same tech.
...
- **NEVER pip install `composio` into Hermes' own venv.** Verified this session: PyPI `composio` 0.18.x requires `openai>=2.48`, but hermes-agent pins `openai==2.24.0`. Installing it into `.../hermes-agent/venv` silently upgraded openai and broke hermes-agent (imports failed). Always install Composio in the isolated venv at `OPERATOR_HOME/AppData/Local/hermes/composio-venv` (created this session). The PyPI `composio` package is SDK-only — there is NO CLI binary (`composio-cli` npm package 404s; `python -m composio` errors with "No module named composio.__main__"). Interactive `composio login` / `add linkedin` OAuth must be completed by operator in a browser; Alfred then calls the SDK or the Composio MCP server.

- **The bundled scripts had two real bugs, both fixed 2026-08-07. If an older copy is
  used elsewhere, re-apply these:**
  - `md2pdf_ats.py` leaked the leading `# ` into the rendered name (`# OPERATOR_NAME`)
...
- **Do not pad keywords to reach 100%.** A realistic MASTER resume scores ~80% against a
  ...
  are safe to ignore, and GENUINE gaps in operator's background (aws, redis, kafka,
...
- **SCREEN ON COMPENSATION BEFORE APPLYING, not after.** Read the posted band off the
  ...
  unless operator says otherwise. On 2026-08-07 an application went to Codemonk at a posted
...
## LinkedIn automation — durable operating rules (2026-08-09, apply to ALL LinkedIn-driving work)
These govern the puppeteer/9222 browser rail (see the `linkedin-easy-apply` skill, now curator-managed, and `apply_one.cjs` driver in `OPERATOR_HOME/job-apply/`). They are HOW to operate for operator.
- **Restriction-safe FIRST.** operator's account was temporarily restricted (until 00:52 UTC Aug 8
...
- **NEVER use `computer_use`/SOM/UIA on the LinkedIn 9222 Chrome (HARD, 2026-08-16).** It
  ...
  via raw CDP (puppeteer-core `connect` to `ws://127.0.0.1:9222`) + `vision_analyze` on
  ...
  `references/linkedin-easy-apply-cdp-fixes-2026-08-16.md` for the validated CDP driver fixes.
- **Persist ALL Chrome sessions** across turns (9222 LinkedIn profile at
  `OPERATOR_HOME/chrome-cdp-profile`, 9333 automation-chrome). Relaunch with
...
- **NEVER kill the WhatsApp bridge** (`node .../whatsapp-bridge/bridge.js --port 3000`). It is
  operator's PRIMARY comms channel (self-chat). Before killing any `node` PID, inspect its
...
- **Job crons + the apply loop are governed across BOTH channels — a desktop pause is NOT sovereign.
  (CRITICAL governance wall, verified 2026-08-23.)** The WhatsApp channel (`33406473744457@lid`,
  "Cyberbatman") is ALWAYS connected. When operator sends a WhatsApp message implying "keep applying"
...
- **LinkedIn "AI job search" SPA gotcha (2026-08-09, CORRECTED 2026-08-09):** scripted left-list
...
- **Resume to select for EA apply = `operator_Biswas_Resume_ATS.pdf`** (NOT "Kibhu_...AIS" — that
...
- **Screenshot hygiene (operator, 2026-08-09):** the 1s poller makes ~130 PNGs/session (~140KB
...
- **Safe retest protocol (agreed 2026-08-09, after 2 detection trips):** when operator says "go"
  post-cooldown, run `printf '[]' > applied_fly.json` then
  `ALLOW_RUN=true QUERY="Senior Software Engineer" LOCATION="India" MAX_APPLY=3 DELAY_MS=8000 node apply_fly.cjs`
...
## LinkedIn Easy Apply — PROFILE-REASONING answering (2026-08-17 HARD CORRECTION)
...
(click/type via puppeteer-core @ 9222); the "brain" that decides each answer is operator's real
profile + agent reasoning.

- **Architecture (validated this session, `OPERATOR_HOME/job-apply/apply_one.cjs` + `cdp_helper.cjs`):**
  a `PROFILE` object holds operator's verified data; `answer(label)` reasons over it with instant
  ...
  into fields). See `references/linkedin-easy-apply-profile-reasoning-2026-08-17.md`.
- **NEVER expose "Alfred"/Hermes persona in application answers.** Keep that identity internal;
  answer as OPERATOR_NAME. (operator explicit 2026-08-17.)
- **Isolated tab per call, with a 75s watchdog. (CORRECTED 2026-08-20 — the old
...
- **Fresh-window recursive collection (operator 2026-08-17):** searches use `f_TPR` in the URL
...
- **`empty_modal` is ALMOST ALWAYS a BOT-DETECTION BUG, not a LinkedIn glitch. (CORRECTED 2026-08-17.)**
  ...
  a true LinkedIn-side glitch. See `references/linkedin-easy-apply-cdp-detection-fixes-2026-08-17.md`.
- **`NO_EA_BUTTON` usually means ALREADY APPLIED.** LinkedIn removes the EA button once submitted and
  ...
  applied) — do NOT treat as failure or re-attempt. (Validated 2026-08-17: two "failed" jobs were in
...
- **Relevance gate (operator JS/frontend focus):** RELEVANT = `/javascript|typescript|front[- ]?end|full[- ]?stack|node\.?js|react|vue|angular|ui developer|web developer/i`; IRRELEVANT = `/embedded|firmware|automotive|autosar|golang|java(?!script)|python(?!.*react)|rust|spring boot|.../i`. Skip non-matching (e.g. 777 Trinity "Embedded Software Engineer" — not his profile). India JS/frontend EA volume in any 5h window ≈ 1 job; widen `f_TPR` for more.
- **Chrome relaunch on 9222 (Windows gotcha):** `bash` `kill -9 <pid>` can NOT reach the Chrome
  process handle. Use `taskkill /PID <pid> /F /T` (found via `netstat -ano | grep :9222`). Then
  relaunch: `"C:/Program Files/Google/Chrome/Application/chrome.exe" --remote-debugging-port=9222
...
## References
- `references/linkedin-easy-apply-cdp-detection-fixes-2026-08-17.md` — CORRECTED EA detection/classification: whole-document field/button scan (don't scope to first `div[role=dialog]`), `NO_EA_BUTTON`=already-applied, `f_TPR` URL window (not post-time text), radio question = preceding sibling of `<fieldset>`, Yes/No "years with X"→"Yes", relevance gate, screenshot hygiene. Vision-proven this session. **UPDATED 2026-09-01:** dialog is `<dialog open>`, not `div[role=dialog]`.
- `references/linkedin-easy-apply-profile-reasoning-2026-08-17.md` — PROFILE-reasoning EA answerer design: PROFILE object, `answer()` reasoning + LLM fallback (text-only), catalog of answer() rules, single-tab + fresh-window collection. Validated by 16+ submissions this session. **UPDATED 2026-09-01:** replaced by lookup table (`getAnswer`) — LLM too slow, lookup table instant.
...
- `references/linkedin-easy-apply-cdp-fixes-2026-08-16.md` — PROVEN CDP-driver fixes (modal detection, scroll-before-click, "Next" match, contact pre-fill, answer() gaps, custom-radio div click, stuck detection). **SOM/computer_use HANGS LinkedIn — use raw CDP + vision, never SOM.**
...
- `references/ats-resume-pattern.md` — ATS-maximized resume structure + the Okta PAM case study (verified keyword coverage ~96%).
- `references/linkedin-composio.md` — LinkedIn-via-Composio setup, venv isolation, and the Okta referral workflow (operator authorized LinkedIn interaction through Composio, Aug 2026).
