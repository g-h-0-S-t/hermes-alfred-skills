...
description: "Drive LinkedIn 9222 Chrome via CDP; computer_use freezes it."
version: 1.0.0
...
- Any request to apply on LinkedIn, scrape LinkedIn jobs, navigate the LinkedIn job list,
  or fill a LinkedIn Easy Apply form on THIS machine (MSI GS65, 9222 Chrome).
...
operator's LinkedIn lives in a dedicated Chrome at `http://127.0.0.1:9222`
(profile `OPERATOR_HOME/chrome-cdp-profile`). Drive it with **raw CDP only**.
The single most important lesson from 2026-08-15: **cua-driver / `computer_use` /
...
- Symptom: browser main thread stops answering. `curl -s -m8 http://127.0.0.1:9222/json/version`
...
- CAUSE B (CDP page crash on load — the one that actually bit the EA pipeline, 2026-08-16):
  The 9222 profile had ~30 extensions loaded AND Chrome's default `/dev/shm` on this machine
  ...
  without losing the login: relaunch the SAME 9222 profile with `--disable-extensions
  ...
  sustained CDP work is fine. Keep ONE rule: if 9222 is ever relaunched, use the flag set in
...
- **Concrete repro that broke the bot (2026-08-15):** the EA driver's `collectJobs` did
  **14 sequential keyword `page.goto` calls + a 6× `window.scrollBy(0,1500)` loop per query**
  ...
  that kills it.
- **Keep the EA driver SIMPLE (2026-08-15 lesson).** A "universal" rewrite that (a) added an
  ...
  - **2026-08-17 REFINEMENT:** the bot must REASON from operator's profile (not a static table) and
    a CONSTRAINED LLM oracle is now permitted — see "EVOLUTION 2026-08-17" below. The destructive
...
"C:/Program Files/Google/Chrome/Application/chrome.exe" --remote-debugging-port=9222 \
  --user-data-dir="OPERATOR_HOME/chrome-cdp-profile" --disable-extensions \
  --no-sandbox --disable-gpu --disable-software-rasterizer \
  --disable-dev-shm-usage --no-first-run \
  --hide-crash-restore-bubble --disable-backgrounding-occluded-windows \
  --disable-renderer-backgrounding --disable-background-timer-throttling
```
**The `--disable-extensions` + `--disable-dev-shm-usage` flags are MANDATORY** (2026-08-16
...
Health probe before any work: `curl -s -m8 http://127.0.0.1:9222/json/version` must return
...
## Easy Apply specifics (2026-08-15)
...
- **`f_TPR` IS a reliable posted-within filter (CORRECTED 2026-08-17).** Earlier notes claimed
...
- **Supply reality:** the "EA + Bengaluru + last 5h" intersection is often EMPTY. Do not
...
- Each run: ensureBrowser (relaunch if 9222 down) -> ONE light scrape (`scrape_bengaluru.cjs`)
  -> filter `easy && bengaluru && <=5h && score>=60` -> apply at most ONE job via
  `apply_one.cjs` -> log to `ea_applied_b.json` / `ea_skip_b.json` (never re-applies).
- Reuse `OPERATOR_HOME/job-apply/cdp_helper.cjs` (`withPage` helper) and run Node with
  `PYTHONPATH`/`PYTHONHOME` UNSET: `env -u PYTHONPATH node script.cjs`.

## WORKING EA-DRIVER PATTERN (2026-08-16, proven end-to-end)
A single puppeteer-core driver (`OPERATOR_HOME/job-apply/linkedin_apply_30min.cjs`) now
...
- **Contact step is PRE-FILLED from the profile.** Email/First name/Last name/Mobile/Phone-country
  ...
  report `prefilled`. Only fill when empty. The "Phone country code" select holds "India (+91)"
  — a country, not a phone number; never stuff the phone digits into it.
- **NEVER expose the "Alfred" persona/identity when filling job applications.** operator's internal
  agent name ("Alfred") is private. On "Which AI tools do you use daily?" and similar, the answer
  must name the TOOLS (Hermes, omniroute, Antigravity, Kilo Code, Cursor, Ollama/LM Studio) — never
  the "Alfred" identity or "my agent". Keep that internal. The full AI-tools phrasing and the rest
  of the profile-driven answer bank live in `references/ea-answer-map.md`.
- **Fill from the REAL profile; only queue when genuinely unknown (2026-08-16 correction).**
  Skill-year questions ("years with Python/Java/Angular/PostgreSQL…"), total-experience,
  notice-period, and compensation fields ARE answerable from operator's verified profile data
  (the `SKILL_YEARS` map in the driver + CTC 8600000/expected 5000000). These are REAL facts,
  ...
  company-specific question like "why do you want to work here"). operator's standing instruction:
...
- **Resume:** never upload a file; the in-account stored ATS resume auto-attaches. If a resume
  picker is shown, select "operator_Biswas_Resume_ATS.pdf" only; otherwise leave default untouched.
...
- **Relevance pre-filter (operator's profile focus, 2026-08-17):** operator is a **JavaScript / frontend /
  ...
  search surfaced. Reality check: in any 5h window only ~1 relevant JS/frontend EA role posts in India —
...
## FORM-FILL REACT GOTCHAS (2026-08-16, verified while fixing the EA driver)
...
- **Radios/checkboxes: click the VISIBLE `role=radio`/`role=checkbox` DIV, not the hidden
  ...
  does nothing — React's handler is on the div. CRITICAL (2026-08-17, re-verified): an
  **in-page `el.click()` / synthetic `.click()` on the div is ALSO a no-op** — React ignores it.
  ...
  - **Radio question-climb (pick the option) — CORRECTED 2026-08-17:** the ancestor-climb method
  ...
  - **Yes/No radio for "years of experience with X" — NEW PITFALL 2026-08-17:** LinkedIn renders
...
- **Consent/agreement CHECKBOXES must be auto-checked (2026-08-17, NEW):** the driver handled text/
...
- **ALREADY_APPLIED must NOT be classified as `failed` (2026-08-17, NEW):** when a job is already
...
## EVOLUTION 2026-08-17 — profile-reasoning brain + constrained LLM fallback
operator's explicit correction: *"Why are you using a script? I expect Alfred to always actively
answer the questions based on my personal and professional data and life's information!"* The
bot's "brain" must be operator's real profile (held by the agent), not a hand-maintained
...
1. **Instant regex** for structured fields (salary/CTC-with-LPA-awareness, notice-period, years,
   ...
   AI-tools→name the TOOLS not "Alfred", project-link→GitHub). Zero latency.
...
- This is the correct reading of "evolve your skills, Alfred — let's go.": make the answering
  profile-aware, then add an LLM fallback so novel questions self-resolve instead of dying.

### Single-tab discipline (operator, 2026-08-17)
...
### WRONG-DIALOG SCOPING — false "empty modal" / "no submit" (2026-08-17, CORRECTED)
...
### Vision-confirm + screenshot cleanup (operator, 2026-08-17, HARD)
...
use a Hermes **cron job** (every 15 min, `deliver=all`): each run heals 9222 (`node heal_9222.cjs`
...
and model 503s. `heal_9222.cjs` probe: `curl -s -m5 http://127.0.0.1:9222/json/version`.
- references/ea-answer-map.md — profile-driven answer bank, radio click recipe, "no Alfred" rule.
- references/ea-profile-reasoning.md — 2026-08-17 evolution: profile object, 3-tier dispatch,
...
- references/ea_2026-08-17_fixes.md — session receipts: wrong-dialog false "empty modal" proof, radio
...
- references/ea_2026-08-17_checkbox_alreadyapplied.md — consent-checkbox auto-check snippet + already_applied
...
- CANONICAL DRIVER (2026-08-17, current working pair): `OPERATOR_HOME/job-apply/apply_one.cjs`
  (applies ONE job URL, fills every field from the verified profile + answer bank, submits, returns
  JSON with `submitted`/`confirm`/`unanswered`) and `OPERATOR_HOME/job-apply/run_recursive.cjs`
  ...
  (mirrored in `references/ea-answer-map.md`). NOTE: 9222 background runs in Hermes' non-tty shell
...
- OUTDATED / BROKEN: the user-owned `linkedin-easy-apply` SKILL was rewritten into a complex
  ...
  The SKILL itself is USER-OWNED — Alfred cannot `skill_manage` it. operator must run
  `hermes curator adopt linkedin-easy-apply` to let Alfred own/patch it. NOTE THE FILE/SKILL
  DISTINCTION: the SKILL is outdated, but the SCRIPT `OPERATOR_HOME/job-apply/linkedin-easy-apply.cjs`
