# EA Answer Map — profile-driven field answers (XXXXXXX)
...
- Location: XXXXXXX (prefer for location dropdowns).
- Join immediate -> notice period / join-in-days = 0.
- AI tools (NO "Hermes Agent" mention): "Daily: Hermes, omniroute (model routing), Antigravity (Google
...
- /work (from )?(the )?office|work 5 days|office in XXXXXXXX|onsite|in.?office/ -> Yes
...
- Compensation "in INR LPA" maps to 86/50, NOT XXXXXXX/XXXXXXX.
- Background run in Hermes non-tty shell dies ("stdin is not a tty", exit 1) -> run foreground.
- NEVER mention "Hermes Agent" in any application answer. Name the tools, not the persona.
