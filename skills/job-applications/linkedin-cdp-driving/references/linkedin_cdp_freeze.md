# LinkedIn LINKEDIN_PORT Chrome freeze — evidence & diagnosis recipe

## Symptom
Browser stops responding during LinkedIn automation. `curl -s -m8 http://127.0.0.1:LINKEDIN_PORT/json/version`
returns `http=000` (empty) while `netstat -ano | grep :LINKEDIN_PORT` still shows LISTENING.
...
## CAUSE B — CDP page crash on load (the one that broke the EA pipeline, XXXXXXX)
...
Root cause: the LINKEDIN_PORT profile had ~30 extensions loaded AND Chrome's `/dev/shm` on this machine
...
losing the login: relaunch the SAME `chrome-profile` with `--disable-extensions
...
Relaunch the LINKEDIN_PORT profile with the hardening flag set above; then sustained collect+apply is safe.
