...
Captured 2026-08-15 after a "universal" rewrite broke the bot. The simple version is what
...
2. **Collect marathon** — `collectJobs` did 14 sequential `page.goto` keyword searches + a
   ...
   froze the 9222 Chrome (Windows Event 1002 "Application Hang", CDP `http=000`).
...
- **Profile-only fill.** `applicant.profile.json` holds contact / `resumeName` / `ctc`
  (current 8600000, expected 5000000, notice 0) / `locationPref` Bengaluru / `answers`
...
- **Open EA via a REAL mouse click at the button center** — synthetic `clickText` /
  ...
  (Note 2026-08-16: even a trusted `el.click()` via puppeteer sometimes needs a RETRY — the EA
...
- **Resume: select stored ATS resume radio only — NEVER upload a file.** The default resume
  `operator_Biswas_Resume_ATS.pdf` is already set in his account.
- **Collect = multiple keywords in ONE browser session is NOW FINE** (2026-08-16 correction):
  ...
  9222 profile's ~30 extensions + tiny `/dev/shm` crashed every page. Keep collect reads light
...
- **Unanswered required field -> GAP, job skipped/queued** (logged to `needs_review_30min.json`
  for fields needing operator's real numbers, e.g. tool-specific years-of-experience or comp
...
`<wsUrl>` = `webSocketDebuggerUrl` from `http://127.0.0.1:9222/json/version`.
