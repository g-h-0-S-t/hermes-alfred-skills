...
operator's `whatsapp` channel (`33406473744457@lid`, "Cyberbatman") is the PRIMARY
...
Verified 2026-08-23: paused the 3 active crons from desktop; next turn they were
...
2. Kill the LIVE loop so the watchdog can't relaunch it:
   ...
   → `taskkill /PID <pid> /F /T`. Pausing the watchdog (`355e44e24008`) alone is NOT
...
(the `355e44e24008` watchdog does this every 30m), and clear the suspension memory
note.

## Evidence / debug trail (where to look)
- Cron state file: `OPERATOR_HOME/AppData/Local/hermes/cron/jobs.json`
  → per-job `enabled`, `state`, `paused_at`, `updated_at`.
- WhatsApp re-enable source: `OPERATOR_HOME/AppData/Local/hermes/logs/gateway.log`
  → `inbound message: platform=whatsapp user=Cyberbatman ... msg='...'`.
  Timestamps here correlate with `jobs.json` `updated_at` flips.
- Channel connection state: `OPERATOR_HOME/AppData/Local/hermes/gateway_state.json`.
- NEVER kill the WhatsApp bridge PID (operator's primary channel) — see LinkedIn
