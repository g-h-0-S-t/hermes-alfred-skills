# Kilo free gateway — working models + endpoint (verified 2026-08-15)
...
- **Text / classification / drafting**: `nvidia/nemotron-3.5-lightning:free`
  - Live check 2026-08-15: HTTP 200, ~0.7s, clean JSON. (Non-free `nvidia/nemotron-3.5-lightning` → 401 PAID_MODEL_AUTH_REQUIRED.)
- **Vision** (accepts `image_url` in `messages[].content` array): `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free`
  - This is the "omni" = vision+text model. Live check 2026-08-15: sent a 1x1 red PNG, got `{"color":"red"}` in ~3s. Screenshots a form control and returns `{"choice":"<option>"}` correctly.
...
- This mirrors the Wraith Engage `core/ai.js` `_kiloGenerate` shape (model + messages, chat/completions URL derived from `kiloModelsUrl` origin).
