...
## When to use
- operator asks to automate LinkedIn Easy Apply AND the CDP/puppeteer-on-LINKEDIN_PORT path is crashing (renderer dies on the EA iframe). This happened repeatedly XXXXXXX-15.
- Building a distributable tool: a Chrome extension the user loads on linkedin.com, uploads their resume, clicks Apply; the extension scans jobs and AI-fills the form. operator explicitly proposed this ("like XXXXXXX Engage").
- Any task to make LinkedIn EA robust + 0%-failure.

## Architecture (ACTUALLY BUILT + DEBUGGED XXXXXXX-15 — use this, not the old XXXXXXX Shadow-DOM plan)
A **Chrome MV3 extension runs inside LinkedIn's own page** (content script, same-origin to the EA form). This avoids the CDP/cross-origin-iframe renderer crash that kills puppeteer-on-LINKEDIN_PORT. The real repo is `YOUR_GITHUB_USERNAME/linkedin-easy-apply-ext` (local: `XXXXXXX/linkedin-easy-apply-ext`). Files:
...
## Build/debug pitfalls (all hit + fixed XXXXXXX-15 — see chrome-mv3-extension-dev for the full list)
...
- Resume picker: **select the stored ATS resume only — never upload a file** (upload corrupts submissions; user's stored resume is correct).
...
## Pitfalls
- **CDP/puppeteer-on-LINKEDIN_PORT crashes on LinkedIn's EA iframe** on user's machine (renderer dies ~6-40s into interaction; every flag combo — `--disable-gpu`, `--no-sandbox`, `--in-process-gpu`, `--disable-features=site-per-process` — failed). Do NOT sink time into Chrome stability flags; build the extension instead. (Lesson from XXXXXXX-15: ~9 live attempts all died; the fill engine was proven correct but the browser couldn't survive the EA iframe.)
- Never fabricate answers for missing required fields — pause for human review (user's standing rule; fabricated education %/GPA burned him before).
...
- `taskkill /F /IM chrome.exe` in the terminal tool is auto-approved; use it to clear zombie Chrome procs, but a fresh launch via `terminal background=true` is the reliable way to bring LINKEDIN_PORT up (MSYS `start` mangles the Windows path).

## References
- references/XXXXXXX-architecture.md — MV3 blueprint extracted from YOUR_GITHUB_USERNAME/XXXXXXX-marketing-plugin
- references/kilo-models.md — working free model IDs + endpoint shape + live probe

## Setup

Chrome MV3 extension for LinkedIn Easy Apply (alternative to CDP).
...
- Kilo API key (free tier, no sign-in required)

**Placeholders used:** XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX
