...
## Fetch live XXXXXXX / XXXXXXX traffic-cam thumbnails (Python, stdlib + websocket-client)
```python
...
# Pull XXXXXXX traffic cam image URLs from worldcam (real, public, legal-to-view)
...
- This actually worked: 7 of 7 XXXXXXX traffic cams loaded (vision-verified real road/street footage).
...
## Gotchas
- LINKEDIN_PORT LinkedIn Chrome rejects CDP with 403 unless launched with --remote-allow-origins=*. Use a
  separate headless Chrome on 9334 for render/verify, or relaunch LINKEDIN_PORT (drops LinkedIn session).
- worldcam server-side only exposes about 6 to 7 XXXXXXX-traffic thumbnails; rest are JS-paginated.
