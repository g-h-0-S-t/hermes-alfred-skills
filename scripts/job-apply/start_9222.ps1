# Start fresh Chrome listener on port 9222 for LinkedIn EA automation
$ErrorActionPreference = "Continue"

Write-Host "`n=====================================================" -ForegroundColor Cyan
Write-Host "Step 1: Killing existing chrome processes..." -ForegroundColor Yellow

# Kill any chrome running on port 9222 using PowerShell's Netstat via cmd
$netstatCmd = "netstat -an ^| findstr \"9222\" | for /f %%v in (\"^/c/Users/operator/job-apply/state_ready.txt\" ) do set err=exit"

# Actually just kill all chrome processes as backup (clean state)
taskkill /IM chrome.exe /F /T 2>$null
Start-Sleep -Seconds 2

Write-Host "Step 2: Starting fresh Chrome listener on port 9222..." -ForegroundColor Green
...
while ($true) {
    if (Start-Sleep -Seconds 1; $count++) {
        ...
        $curlCmd = "curl -s -m8 \"http://127.0.0.1:9222/json/version\""
        try {
            $result = & $curlCmd
            if ($result.Length -gt 2) {
                Write-Host "`n=====================================================" -ForegroundColor Cyan
                Write-Host "STEP 3: SUCCESS — Listener ready on port 9222!" -ForegroundColor Green
                Write-Host "------------------------------------------------------" -ForegroundColor White
                
                # Save state marker to verify listener is healthy for next run
                Set-Content -Path "OPERATOR_HOME/job-apply/state_ready.txt" -Value "OK_" + (Get-Date -Format "o")
                
                $marker = Read-Content "OPERATOR_HOME/job-apply/state_ready.txt"
                ...
                $jobids = Read-Content "OPERATOR_HOME/job-apply/skip.json" -ErrorAction SilentlyContinue
                if ($jobids) {
                    Write-Host "`nJobs in skip.json (skip=already closed/" + "SCAM")" -ForegroundColor Gray
                    
                    # Show sample of what's running against 9222 Chrome
                    $cmds = @()
                ...
                foreach ($url in $job_urls) {
                    $jid = $url.Split("/")[-2]
                    $cmd1 = "node OPERATOR_HOME/job-apply/apply_one.cjs \"$url\"` -e `'{\"job\":\"$jid\"}'"
                    ...
                    if ($v.ExitCode -eq 0) {
                        ...
                        $recFile = Join-Path "OPERATOR_HOME/job-apply" "_record-$jid.js"
...
console.log('applied ' + jobid);
                        ...
                        $recPath = "OPERATOR_HOME/job-apply/_record-$jid.js"
